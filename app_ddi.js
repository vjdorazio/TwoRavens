...
function bivariatePlot(x_axis,y_axis)
{
   /* for(var i=0; i<1000;i++)
    {

        console.log("x_axis is :"+ x_axis[i]);
        console.log("y_axis is :"+ y_axis[i]);
    }
    */
   ...
   // var NA=-1;
   // var x_axis=[1,15,12,32,424,42,6,5,34,29,982,87,67,56,43,44,5,6,666,NA,66];
   // var y_axis=[4,65,12,32,44,81,61,51,314,219,82,7,37,56,73,14,52,9,66,NA,616];
    ...
    var randomizeData = function() {
        ...
        var x, y, col, row,x1,y1;
        for (var i = 0; i < x_axis.length; i++) {
            if (isNaN(x_axis[i]) || isNaN(y_axis[i])) {
                console.log("NA is at : "+i)
            } else {
                // x = Math.random() *size1;
                //y = Math.random() *size1;
                x1 = x_axis[i];
                y1 = y_axis[i];
                col = Math.min(Math.floor(x1 / size1 * numCols), numCols - 1);
                row = Math.min(Math.floor(y1 / size1 * numRows), numRows - 1);
               ...
               // console.log("for the i: " + i + " x : " + x1 + " y: " + y1);
              //  console.log("for the i: " + i + " col : " + col + " row: " + row);
    ...
    var createScatterplot = function() {
        ...
        scatterplot.selectAll("circle").data(data).enter().append("svg:circle").attr("cx", function(d, i) {
            ...
            return d.y;
        }).attr("r", size/100).attr("ind", function(d) {
            return d.ind;
        }).on("mouseover", function(d) {
            onPointOver(this, d);
        }).on("mouseout", function(d) {
    ...
    d3.select("scatterplot").append("text"," We have NA values in the data.");
