/* The actor menu cannot have the full list of actors, but still needs to remembers filters applied over the entire dataset.
 ...
 --Specification for [SELECTED], the data structure that describes the elements in a node, or group of actors.
	...
	When a checkbox is clicked in the actor list, a {$Source: 'AFG'} rule is or'ed to root.
		These entries are collapsed to one $in: [] statement by the query builder in app.js.
...
function updateSVG() {
    ...
    function createLink(d) {
        ...
        var linkExist = actorLinks.filter(function (linkItem) {
...
function collide(node) {
    ...
    return function (quad, x1, y1, x2, y2) {
        if (quad.data && (quad.data !== node)) {
            var x = node.x - quad.data.x, y = node.y - quad.data.y, l = Math.sqrt(x * x + y * y),
                r = actorNodeR + actorNodeR + actorPadding;
